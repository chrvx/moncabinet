...
- **`evenement`** tracks what actually happened on a dossier orally (a meeting, a phone call, a hearing) as opposed to `echeance`, which tracks what's planned. Unlike `contact` or `document`, it uses no inheritance pattern — every event has the same shape (date, optional duration, mandatory free-text `contenu`), and only a `type_evenement` reference (an editable lookup table, like `matiere` or `categorie_echeance`) varies. `contenu` is `NOT NULL` with no exception, including for the "Autre" type — the whole point of the table is that an oral exchange always leaves a written trace. There's no real deletion: an event is instead cancelled (`annule_le`/`annule_par`/`motif_annulation`, reversible via "Réactiver", on the same model as `douteux` on coordonnées) and hidden from the default listing rather than removed. Every write that modifies an existing event (content correction, moving it to another dossier, cancellation, reactivation) is preceded, in the same transaction, by a copy of the current row into `evenement_historique` — done in explicit Python (`app/repositories/evenements.py::_historiser`), not a PostgreSQL trigger, since a 2-3 user cabinet has no code path that bypasses the repository layer but not an equally-privileged trigger. `modifie_par`/`modifie_le` are always displayed in the UI, not just stored, so a correction is never silent. Moving an event to a different `dossier_id` is a separate, more sensitive operation (`changer_dossier`) gated to `avocat`/`collaborateur` accounts, unlike a plain content correction which any connected account can make. The "Chronologie" view merges `evenement` and `document` listings in application code (no new table) since e-mails already live in `document`/`document_email` and aren't duplicated into `evenement`.
